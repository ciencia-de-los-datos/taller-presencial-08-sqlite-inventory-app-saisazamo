...
#
# Logging setup (guarda bitacora de lo que va pasando)
#
logging.basicConfig(
...
def get_db():
    ...
    if "db" not in g:           #si la BD no esta en g (variable donde almaceno todas las vairables que se comparten en flask), intente;
...
@app.teardown_appcontext        #Cuando la app salga, llame esta funcion:
...
@app.route("/requestparts", methods=["POST"]) #Cuando ingrese a la aplicacion:
def requestparts():
